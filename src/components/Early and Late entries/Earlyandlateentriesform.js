...
import { TIMECARD, REPORT } from '../../serverconfiguration/controllers';
...
export default function EarlyAndLate() {
    ...
    const handleSave = async () => {
        try {
            const filteredEmployees = timecard.filter((r) => r.pnBranchid == pnBranchid && r.status == 'P');
            ...
            if (filteredEmployees.length > 0) {
                for (const employee of filteredEmployees) {
                    ...
                    const response = await postRequest(ServerConfig.url, REPORT, { query: query });
                    console.log('Response:', response);
                }
            } else {
                console.error('No employees found for the selected branch or status is not "P"');
            }
        } catch (error) {
        ...
        <div>
            <Grid style={{ padding: "80px 5px0 5px" }}>
                <Card style={{ maxWidth: 1200, margin: "0 auto" }}>
                    <CardContent>
                        <Typography variant='h5' color='S- Light' align='center'>Early and Late Entries</Typography>
                        <form>
                                ...
                                <Grid item xs={12} sm={12}>
                                    <FormControl fullWidth>
                                        ...
                                        <select
                                            ...
                                            {timecard.map((e) => <option key={e.Id}>{e.pnCompanyid}</option>)}
                                        </select>
                                    </FormControl>
                                </Grid>
                                ...
                                <Grid xs={12} sm={12} item>
                                    <FormControl fullWidth>
                                        ...
                                        <select
                                            ...
                                            <option value="">Select</option>
                                            {timecard.filter((e) => e.pnCompanyid == pnCompanyid).map((e) => <option key={e.Id}>{e.pnBranchid}</option>)}
                                        </select>
                                    </FormControl>
                                </Grid>
                                    ...
                                    <table style={{ width: '100%' }}>
                                        <thead>
                                            <tr>
                                                ...
                                                <th style={{ textAlign: 'left', padding: '10px' }}>Early Out <Checkbox checked={showEarlyOut} onChange={handleEarlyOutChange} /></th>
                                                <th style={{ textAlign: 'left', padding: '10px' }}>Late Entries <Checkbox checked={showLateFields} onChange={handleLateEntriesChange} /></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {timecard.filter((r) => r.pnBranchid == pnBranchid && r.status == 'P').map((e, index) => (
                                                <tr key={e.empCode}>
                                                    <td style={{ textAlign: 'left', padding: '10px' }}>{e.empCode}</td>
                                                    <td style={{ textAlign: 'left', padding: '10px' }}>{e.empName}</td>
                                                    <td style={{ textAlign: 'left', padding: '10px' }}>
                                                        {showEarlyOut && <FormControl>
                                                            <TextField
                                                                name="Early Out"
                                                                label="Early Out"
                                                                variant="outlined"
                                                                ...
                                                                onChange={(event) => handleInputChange(setEarlyOut, e.empCode, event.target.value)}
                                                                InputLabelProps={{ shrink: true }}
                                                            />
                                                        </FormControl>}
                                                    </td>
                                                            ...
                                                            <div>
                                                                <FormControl style={gapStyle}>
                                                                    <TextField
                                                                        ...
                                                                        onChange={(event) => handleInputChange(setLateIn, e.empCode, event.target.value)}
                                                                        InputLabelProps={{ shrink: true }}
                                                                    />
                                                                </FormControl>
                                                                <FormControl >
                                                                    <TextField
                                                                        ...
                                                                        onChange={(event) => handleInputChange(setLateOut, e.empCode, event.target.value)}
                                                                        InputLabelProps={{ shrink: true }}
                                                                    />
                                                                </FormControl>
                                                            </div>
